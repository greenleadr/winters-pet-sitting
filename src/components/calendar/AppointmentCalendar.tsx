...
import { PlusIcon, CalendarIcon, ListIcon, PawPrintIcon, HomeIcon, DollarSignIcon, TrashIcon, XIcon } from 'lucide-react';
    ...
    <div className="px-0 py-0">
        ...
        <div className="flex rounded-lg border border-gray-200 overflow-hidden">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium transition-colors ${
              view === 'week' ? 'bg-emerald-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium transition-colors border-l border-gray-200 ${
              view === 'agenda' ? 'bg-emerald-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'
            }`}
          >
      ...
      <div className="flex items-center gap-3 px-4 py-2 bg-white border-b border-gray-100">
        <span className="flex items-center gap-1 text-xs text-gray-500">
          <span className="w-2.5 h-2.5 rounded-full bg-purple-600 inline-block" /> Pet Sitting
        </span>
        <span className="flex items-center gap-1 text-xs text-gray-500">
          <span className="w-2.5 h-2.5 rounded-full bg-cyan-600 inline-block" /> Cleaning
        </span>
        <span className="flex items-center gap-1 text-xs text-gray-500">
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-600 inline-block" /> Other
        </span>
      </div>
      ...
      {/* Upcoming list (agenda preview on mobile) */}
      {view === 'agenda' && upcomingAppointments.length === 0 && (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Select
              ...
              onChange={(e) =>
                setFormData((p) => ({ ...p, payment_status: e.target.value as PaymentStatus }))
              }
            />
          </div>
          <Textarea
            label="Payment Notes"
            placeholder="Venmo sent, cash on arrival…"
            rows={2}
          ...
          <Textarea
            label="Notes"
            placeholder="Extra instructions for this appointment…"
            rows={2}
          ...
          <div className="flex gap-3">
            <Button variant="secondary" fullWidth onClick={() => setNewModalOpen(false)}>
              Cancel
            </Button>
        ...
        >
          <div className="space-y-3">
            <div>
              ...
              {selectedAppointment.client && (
                <p className="text-sm text-gray-600 mt-0.5">
                  {(selectedAppointment.client as any).first_name}{' '}
                  {(selectedAppointment.client as any).last_name}
                </p>
              )}
            </div>
            ...
            {selectedAppointment.service_type && (
              <div>
                {selectedAppointment.service_type === 'pet_sitting' && (
                  <Badge variant="purple">Pet Sitting</Badge>
                )}
                {selectedAppointment.service_type === 'house_cleaning' && (
                  <Badge variant="blue">House Cleaning</Badge>
                )}
              </div>
            )}
            ...
            <div className="flex items-center gap-2">
              ...
              {paymentBadge(selectedAppointment.payment_status)}
            </div>
